...
<body>
    <div class="container mt-5">
        <h1>Uprav článok</h1>
        {!! Form::open(['action'=> ['LessonsController@update', $post->id], 'method' => 'POST', 'enctype' => 'multipart/form-data' ]) !!}
            ...
            {{Form::select('theme', array('laravel' => "Laravel", 'css' => 'CSS','html' => 'HTML','js'=>'Javascript', 'undefined'=>'Všeobecné'), $post->theme,['class'=>'custom-select my-1 mr-sm-2'])}}
            {!! Form::label('lesson', 'Témy do učebných materiálov') !!}
            {{Form::select('lesson', array('intro' => "Intro", 'frontend' => 'Frontend', 'oop'=>'OOP', 'backend'=>'Backend'), $post->lesson ,['class'=>'custom-select my-1 mr-sm-2 lessons'])}}
            {!! Form::checkbox('series', 'series' , true, ['id' => 'serial']) !!} <span>Seriál k učebným materiálom</span>
            <br>
            <br>
            @else
